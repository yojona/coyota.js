...
function DrawCollision(object){
    screen.context.save()
    screen.context.fillStyle = "rgba(255, 0, 0, 0.5)"
    screen.context.strokeStyle = "rgba(255, 0, 0, 0.5)";
...
function IsSolid(inst){
    for(let behavior in inst.behaviors){
        if(behavior == "Solid" && inst.behaviors[behavior].enabled){
            return true
        }
    }
    return false
